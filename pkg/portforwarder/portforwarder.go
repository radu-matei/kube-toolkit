...
func New(clientset *kubernetes.Clientset, config *restclient.Config, namespace string, port int) (*k8s.Tunnel, error) {
	...
	t := k8s.NewTunnel(clientset.CoreV1().RESTClient(), config, namespace, podName, port)
